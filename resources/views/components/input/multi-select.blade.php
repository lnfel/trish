<div>
    <select id="select" style="display: none;">
    	@forelse($model as $service)
    		<option value="{{ $service->id }}">{{ $service->name }}</option>
    	@empty
    		<option value="0" disabled="">No services yet</option>
    	@endforelse
    </select>

    <div x-data="multiSelect()" x-init="loadOptions()" class="flex flex-col">
    	<input name="services" type="hidden" x-bind:value="selectedValues()">
    	...
    	<div class="inline-block relative w-64">
    		<div class="flex flex-col items-center relative">
    				...
    				<div class="my-2 p-1 flex border border-gray-200 bg-white rounded svelte-1l8159u">
    					<div class="flex flex-auto flex-wrap">
    						...
    						<div x-show="selected.length    == 0" class="flex-1">
    							<input placeholder="Select services" class="bg-transparent p-1 px-2 appearance-none outline-none h-full w-full text-gray-800" x-bind:value="selectedValues()">
    						</div>
    					</div>
    			...
    			<div class="w-full px-4">
    				<div x-show.transition.origin.top="isOpen()" class="absolute shadow top-100 bg-white z-40 w-full left-0 rounded max-h-select overflow-y-auto svelte-5uyqqj" x-on:click.away="close">
